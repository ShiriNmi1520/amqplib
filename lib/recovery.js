...
function splitConnectionOptions(connOptions) {
  ...
  if (!Object.prototype.hasOwnProperty.call(connOptions, 'recovery')) {
    return {
      connectionOptions: connOptions,
      recovery: null,
    };
...
class RecoveringCore extends EventEmitter {
  ...
  _resolveInitialConnection(model) {
    if (this._initialReady) return;
    this._initialReady = true;
    this._resolveInitial(model);
  }
  ...
  _resolvePendingWaiters(model) {
    ...
    waiters.forEach((waiter) => waiter.resolve(model));
  }
  ...
  async _connect() {
    ...
    try {
      ...
      this._resolveInitialConnection(model);
...
class RecoveringPromiseModel extends EventEmitter {
  ...
  waitForConnect() {
    return this._core.waitForConnect();
  }
...
class RecoveringCallbackModel extends EventEmitter {
  ...
  waitForConnect(cb) {
    makeCallback(this._core.waitForConnect(), cb);
    return this;
  }
...
function connectWithRecoveryPromise(openModel, recovery) {
  ...
  return recovering.waitForConnect().then(() => recovering);
}
...
function connectWithRecoveryCallback(openModel, recovery, cb) {
  ...
  recovering.waitForConnect(function (err) {
    if (typeof cb !== 'function') return;
    if (err) cb(err);
    else cb(null, recovering);
  });
  return recovering;
}
